...
<div id="insert_form">
    <form action="gen1.php" method="post" enctype="multipart/form-data">
    ...
    if(isset($_POST['submit'])){
        if(!empty($_FILES['new_file']) && !empty($_POST['caption'])){ 
            ...
            if($_FILES["new_file"]["error"] > 0){
                ...
                $v1 = move_uploaded_file($_FILES["new_file"]["tmp_name"], $server. "/genI/". $path);
                $mysqli2 = new mysqli($host,$login,$password,$databaseName);
                $query4 = "INSERT INTO `GenI`(`url`,`dTaken`,`caption`) VALUES ('" . $path . "',NOW(),'" . $caption . "')";
...
<?php
    ...
    if (mysqli_connect_error() ){
        ...
        $query1 = "CREATE TABLE IF NOT EXISTS GenI(pid INT(11) NOT NULL AUTO_INCREMENT,url VARCHAR(100),dTaken TIMESTAMP, caption VARCHAR(30), PRIMARY KEY (pid))";
        $mysqli->query($query1);
        $result1 = $mysqli->query("SELECT * FROM GenI");
        if(mysqli_num_rows($result1) == 0){
            for($i=0;$i < sizeof($file);$i++){
                $query2 = "INSERT INTO `GenI`(`url`,`dTaken`,`caption`) VALUES ('" . $file[$i] . ".png', NOW(),'" . $file[$i] . "')";
                $mysqli->query($query2);
            }
        }  
    }
...
?>
    <?php
        $query3 = "SELECT * FROM GenI ORDER BY pid";
        ...
        if($result2 && $result2->num_rows > 0){
            while($array = $result2->fetch_assoc()){
                ...
                echo "<img src='" . $server . "/genI/" . $array['url'] . "' alt='" . $array['caption'] . "' class='all_imgs' id='" . $counter ."'>";
                echo "</button>";
                $counter++;
            }
        }
